...
(function( factory ) {
	...
	var DataTable = function ( selector, options )
	{
		...
		// When creating with `new`, create a new DataTables, returning the API instance
		...
		this.each(function() {
			...
			for ( i=0, iLen=allSettings.length ; i<iLen ; i++ )
			{
				...
				if (
					...
					{
						_fnLog( s, 0, 'Cannot reinitialise DataTables', 3 );
						return;
					}
				}
	...
	var _ext; // DataTables.ext
	var _Api; // DataTables.Api
	var _api_register; // DataTables.Api.register
	var _api_registerPlural; // DataTables.Api.registerPlural
	 ...
	 * create a DataTables, but which are not exclusively used only for DataTables.
	 ...
	 * `DataTables.Api.register()` when building the API.
	 *
	 ...
	 *
	 * @param  {string|node|jQuery|object} mixed DataTables identifier. Can be one
	 *   of:
	 *
	 *   * `string` - jQuery selector. Any DataTables' matching the given selector
	 *     with be found and used.
	 *   * `node` - `TABLE` node which has already been formed into a DataTables.
	 *   * `jQuery` - A jQuery object of `TABLE` nodes.
	 *   * `object` - DataTables settings object
	 *   * `DataTables.Api` - API instance
	 * @return {array|null} Matching DataTables settings objects. `null` or
	 *   `undefined` is returned if no matching DataTables is found.
	 * @ignore
	 */
	 ...
	 *
	 * @class DataTable.Api
	 * @param {array|object|string|jQuery} context DataTables identifier. This is
	 ...
	 *   var api = new DataTables.Api( 'table.dataTable' );
	 */
	...
	/**
	 * Check if a `<table>` node is a DataTables table already or not.
	 *
	 *  @param {node|jquery|string} table Table node, jQuery object or jQuery
	 *      selector for the table to test. Note that if more than more than one
	 *      table is passed on, only the first will be checked
	 *  @returns {boolean} true the table given is a DataTables, or false otherwise
	 *  @static
	 *  @dtopt API-Static
	 *
	 *  @example
	 *    if ( ! $.fn.DataTables.isDataTable( '#example' ) ) {
	 *      $('#example').dataTable();
	 *    }
	 */
	...
	/**
	 * Get all DataTables tables that have been initialised - optionally you can
	 * select to get only currently visible tables.
	 *
	 *  @param {boolean} [visible=false] Flag to indicate if you want all (default)
	 *    or visible tables only.
	 *  @returns {array} Array of `table` nodes (not DataTables instances) which are
	 *    DataTables
	 *  @static
	 *  @dtopt API-Static
	 *
	 ...
	 *      $(table).DataTables().columns.adjust();
	 *    } );
	 */
	 ...
	 *
	 * Note that the `DataTables.settings` object is aliased to
	...
	DataTable.defaults = {
		...
		/**
		 * Replace a DataTables which matches the given selector and replace it with
		 * one which has the properties of the new initialisation object passed. If no
		 * table matches the selector, then the new DataTables will be constructed as
		 * per normal.
		 */
		"bDestroy": false,
		...
		/**
		 * This property can be used to force a DataTables to use more width than it
		...
		/**
		 * Enable vertical scrolling. Vertical scrolling will constrain the DataTables
	 ...
	 * table options. DataTables does not have a single instance for each DataTables
	 * with the settings attached to that instance, but rather instances of the
	 * DataTables "class" are created on-the-fly as needed (typically by a
	 ...
	 * options.
	 *
	 * Note that the `DataTables.ext` object is available through
